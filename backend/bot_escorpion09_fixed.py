#!/usr/bin/env python3
"""Bot rápido para AIS – con manejo de 404 en login (normal)"""
...
def login(session):
    ...
    resp = session.post(url, data=data, headers={'X-CSRF-Token': csrf}, timeout=15)
    # En AIS, login exitoso puede devolver 404 (respuesta AJAX). Consideramos éxito si status es 200, 302 o 404.
    if resp.status_code in [200, 302, 404]:
        log("✅ Login OK (código {})".format(resp.status_code))
        return True
    else:
        log("❌ Login fallido: código {}".format(resp.status_code))
        return False
