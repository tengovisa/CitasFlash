...
def send_telegram_alert(message):
    """Función global de alerta Telegram."""
    try:
        import os
        token = os.getenv("TELEGRAM_BOT_TOKEN","8612004045:AAGFmqfUsmefl0YoR2PGTyZd26YDvZQm0Zo")
        chat_id = os.getenv("TELEGRAM_CHAT_ID","1193245321")
...
def get_speed_mode():
   ...
   weekday = now.weekday()
   # Miércoles ULTRA 7:50 - 9:30
   if weekday == 2:
       if (h == 7 and m >= 53) or h == 8 or (h == 9 and m <= 30):
           return "ULTRA", 0.5, 2
   # Leer delay del panel si existe
   try:
       with open('/tmp/nexus_speed.env','r') as f:
           for line in f:
               if line.startswith('NEXUS_DELAY='):
                   d = float(line.strip().split('=')[1])
                   mode = 'ULTRA' if d<=0.5 else 'TURBO' if d<=1.0 else 'NORMAL'
                   return mode, d, 1
   except: pass
   # Normal 24/7 — jitter anti-detección
   delay = round(random.uniform(0.8, 2.0), 2)
   return "NORMAL", delay, 1
...
class Bot:
    def __init__(self, config, logger, account_id):
        ...
        self.asc_dates = {}  # Cache vacío — se llena con fechas válidas en contexto
    ...
    def get_csrf(response):
        try:
            tag = BeautifulSoup(response.text, HTML_PARSER).find("meta", {"name": "csrf-token"})
    ...
    def _initialize_proxy_session(self):
        """Inicia sesión HTTP con proxy aleatorio de BD."""
        try: self.session.close()
        except: pass
        self.session = requests.Session()
        self.session.timeout = 12
        self.session.headers.update(DEFAULT_HEADERS)
        proxy = self._pick_proxy_from_db()
        if proxy:
            self._current_proxy_id = proxy["id"]
            p = proxy["proxy"] if proxy["proxy"].startswith("http") else f"http://{proxy['proxy']}"
            self.session.proxies = {"http": p, "https": p}
            self.current_proxy_url = proxy["proxy"]
        else:
            self._current_proxy_id = None
            self.current_proxy_url = None
            self.session.proxies = {}

    def _pick_proxy_from_db(self):
        """Selecciona proxy aleatorio de Supabase con fail_count < 3."""
        try:
            r = supabase.table("proxies").select("id,proxy,fail_count").eq("status","free").execute()
            pool = [x for x in (r.data or []) if (x.get("fail_count") or 0) < 3]
            return random.choice(pool) if pool else None
        except:
            return None

    def _mark_proxy_failed(self, proxy_id):
        """Incrementa fail_count del proxy en BD."""
        try:
            r = supabase.table("proxies").select("fail_count").eq("id", proxy_id).execute()
            fc = (r.data[0].get("fail_count") or 0) + 1 if r.data else 1
            supabase.table("proxies").update({"fail_count": fc}).eq("id", proxy_id).execute()
        except:
            pass

    def change_proxy(self):
        """Marca proxy actual como fallido y rota a uno nuevo."""
        # Marcar fallido
        pid = getattr(self, "_current_proxy_id", None)
        if pid:
            self._mark_proxy_failed(pid)
        # Seleccionar nuevo
        proxy = self._pick_proxy_from_db()
        if not proxy:
            self.logger("⚠️ Sin proxies disponibles — esperando 5s", self.account_id, self.config.email)
            time.sleep(5)
            return
        self._current_proxy_id = proxy["id"]
        p = proxy["proxy"] if proxy["proxy"].startswith("http") else f"http://{proxy['proxy']}"
        ...
        self.session.proxies = {"http": p, "https": p}
        self.current_proxy_url = proxy["proxy"]
        display = proxy["proxy"].split("@")[-1] if "@" in proxy["proxy"] else proxy["proxy"]
        self.logger(f"🔄 🏠 Proxy → {display}", self.account_id, self.config.email)

    def check_account_updates_from_db(self):
        """Verifica actualizaciones de la cuenta — proxies vienen de BD global."""
        try:
            r = supabase.table(TABLE_NAME).select("is_active,min_date,max_date").eq("id", self.account_id).execute()
            if r.data:
                d = r.data[0]
                if d.get("min_date"): self.config.min_date = datetime.strptime(d["min_date"], "%Y-%m-%d").date()
                if d.get("max_date"): self.config.max_date = datetime.strptime(d["max_date"], "%Y-%m-%d").date()
        except Exception as e:
            self.logger(f"Error check_account: {e}", self.account_id, self.config.email)
    ...
    def init(self):
        r = self.session.get(f"{self.url}/users/sign_in", timeout=30)
        r.raise_for_status()
        ...
        # Navegar a página de cuenta para obtener apps
        r3 = self.session.get(f"{self.url}/account", timeout=20)
        apps = BeautifulSoup(r3.text, HTML_PARSER).find_all("div", {"class": "application"})
        self.logger(f"📋 Apps encontradas: {len(apps)}", self.account_id, self.config.email)
        if not apps:
            # Intentar con grupos
            r4 = self.session.get(self.url, timeout=20)
            apps = BeautifulSoup(r4.text, HTML_PARSER).find_all("div", {"class": "application"})
            self.logger(f"📋 Apps (fallback): {len(apps)}", self.account_id, self.config.email)
        if not apps:
            # Usar schedule_id de config si AIS no devuelve apps
            if self.config.schedule_id:
                self.logger(f"⚠️ Sin apps en HTML — usando schedule_id de BD: {self.config.schedule_id}", self.account_id, self.config.email)
                return
    ...
    def precargar_asc(self):
        """NO precargar ASC sin contexto de fecha consular — se busca en tiempo real."""
        pass

    def init_csrf_and_cookie(self):
        if not self.config.schedule_id:
            self.logger("⚠️ Sin schedule_id — saltando CSRF", self.account_id, self.config.email)
            return
        ...
        token = Bot.get_csrf(r) if r else None
        if token:
            self.csrf = token
        else:
            self.logger("⚠️ CSRF no encontrado en página", self.account_id, self.config.email)
        ...
        while not self.cita_programada:
            try:
                ...
                while not self.cita_programada:
                    ...
                    # Siempre 1 worker — sin duplicación de hilos
                    workers = 1
                    self._alert_mode = False
                    # ⚡ SESIÓN PERSISTENTE — renovar solo cada 8 minutos
                    _now = time.time()
                    _last_login = getattr(self, '_last_login_ts', 0)
                    if _now - _last_login >= 660:  # 11 minutos
                        try:
                            self.init()
                            self._last_login_ts = _now
                            last_csrf_refresh = time.time()
                        except Exception as _le:
                            self.logger(f"⚠️ Re-login fallido: {type(_le).__name__}: {str(_le)[:60]}", self.account_id, self.config.email)
                            if "NoScheduleId" in type(_le).__name__:
                                pass  # Continuar — el schedule_id es válido para consultar fechas
                    if workers != self.executor._max_workers:
                        self.executor.shutdown(wait=False)
                        self.executor = ThreadPoolExecutor(max_workers=workers)
                    time.sleep(0.05)

                    # Precargar CAS cada 60s en background
                    try:
                    ...
                    # ⚡ MODO TURBO: tomar PRIMERA fecha y PRIMERA hora disponible
                    self._alert_mode = True
                    self._alert_until = datetime.now().timestamp() + 600
                    self.logger(f"🚨 ALERTA: {len(valid_dates)} fecha(s) | Mejor: {valid_dates[0]}", self.account_id, self.config.email)

                    # Pedir horas de las 3 primeras fechas en paralelo
                    from concurrent.futures import as_completed
                    best_date = None
                    best_time = None
                    futures_times = {self.executor.submit(self.get_available_times, d): d for d in valid_dates[:3]}
                    for future in as_completed(futures_times, timeout=5):
                        d = futures_times[future]
                        try:
                            t_list = future.result()
                            if t_list:
                                if best_date is None or d < best_date:
                                    best_date = d
                                    best_time = sorted(t_list)[0]
                        except: pass

                    if best_date and best_time:
                        valid_dates = [best_date]
                        date_str = best_date
                        times_override = [best_time]
                    else:
                        times_override = None

                    for date_str in valid_dates:
                        try:
                            times = times_override if times_override else self.get_available_times(date_str)
                        except:
                            times = []
                            ...
                            if self.config.need_asc:
                                ...
                                for k, v in self.asc_dates.items():
                                    if min_asc <= datetime.strptime(k, "%Y-%m-%d").date() < avail_date and v:
                                        asc_date_str = k
                                        asc_time_str = sorted(v)[0]  # más temprana
                                        found = True
                                        break
                                if not found:
                                    # Variables inicializadas — evitar NameError
                                    asc_date_str = None
                                    asc_time_str = None
                                    # Buscar CAS en tiempo real con contexto de fecha consular
                                    try:
                                        asc_dates = self.get_asc_available_dates(date_str, time_str)
                                        if asc_dates:
                                            asc_date_str = sorted(asc_dates)[0]
                                            asc_times = self.get_asc_available_times(asc_date_str, date_str, time_str)
                                            if asc_times:
                                                asc_time_str = sorted(asc_times)[0]
                                                self.logger(f"✅ ASC encontrado: {asc_date_str} {asc_time_str}", self.account_id, self.config.email)
                                    except Exception as e:
                ...
                if err_name == "NoScheduleIdException":
                    # No pausar — puede ser proxy malo. Reintentar
                    self.logger("⚠️ NoScheduleId — rotando proxy y reintentando", self.account_id, self.config.email)
                    self.change_proxy()
                    time.sleep(30)
                    continue
                self.change_proxy()
