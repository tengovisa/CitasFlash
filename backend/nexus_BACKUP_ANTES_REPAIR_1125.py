...
def get_pool(self):
    env=open("/root/.env.citafast").read()
    url=re.search(r"SUPABASE_URL=(.*)",env).group(1).strip()
    key=re.search(r"SUPABASE_KEY=(.*)",env).group(1).strip()
    h={"apikey":key,"Authorization":f"Bearer {key}"}
    r=requests.get(f"{url}/rest/v1/proxies?select=id,proxy,fail_count&is_enabled=eq.true",headers=h).json()
    return r,url,key
def pick_proxy(self):
    pool,url,key=self.get_pool()
...
class Bot:
    def __init__(self, config, logger, account_id):
        ...
        self.cookie = None
        self.proxies_list = self.get_global_proxies()
        self.current_proxy_index = 0
        self.current_proxy_url = None
        self.session = requests.Session()
        self.session.timeout = 12
        self.asc_dates = {}
        self.proxy_fail_count={}; self.proxy_fail_count={}; self._initialize_proxy_session()
    ...
    def get_global_proxies(self):
        ...
        key=re.search(r"SUPABASE_KEY=(.*)",env).group(1).strip()
        h={"apikey":key,"Authorization":f"Bearer {key}"}
        r=requests.get(f"{url}/rest/v1/proxies?select=connection_stringdef _initialize_proxy_session(self):
        import time
        p,url,key=self.pick_proxy()
        if not p:
            self.logger("⚠️ Sin proxies disponibles — esperando 5s", self.account_id, self.config.email)
            time.sleep(5)
            return
        self.current_proxy_url=p["proxy"]
        self.session.proxies={"http":p["proxy"],"https":p["proxy"]}
        self._proxy_meta=(p,url,key)
def _initialize_proxy_session(self):
        import random
        pool=self.get_global_proxies()
        if not pool:
            time.sleep(5); return
        self.proxies_list=[random.choice(pool)]
        if hasattr(self,"global_pool") and self.global_pool:
            self.proxies_list=[random.choice(self.global_pool)]
    ...
    def change_proxy(self):
        if not hasattr(self,"proxy_fail_count"): self.proxy_fail_count={}
        ...
        import random; import random; valid=[p for p in pool if self.proxy_fail_count.get(p,0)<3]; pool=valid if valid else pool; self.current_proxy_index=random.randint(0,len(pool)-1)
    ...
    def precargar_asc(self):
        """Precarga fechas y horas CAS en background para booking instantáneo"""
            asc_dates = self.get_asc_available_dates()
            if not asc_dates:
                return
            # Tomar las 3 fechas más tempranas
            for asc_date in asc_dates[:3]:
                try:
                    times = self.get_asc_available_times(asc_date)
                    if times:
                        self.asc_dates[asc_date] = times
                except:
                    pass
            pass
        ...
        while not self.cita_programada:
            try:
                ...
                while not self.cita_programada:
                    ...
                    if not hasattr(self,'_last_asc_cache'): self._last_asc_cache=0
                    if time.time()-self._last_asc_cache >= 60:
                            self.precargar_asc()
                            self._last_asc_cache=time.time()
                        except: pass
                    try:
                            ...
                            if self.config.need_asc:
                                ...
                                for k, v in self.asc_dates.items():
                                    if min_asc <= datetime.strptime(k, "%Y-%m-%d").date() < avail_date and v:
                                        ...
                                        break
                                if not found:
                                    # Usar cache precargado primero — sin HTTP
                                    if self.asc_dates:
                                        asc_date_str = sorted(self.asc_dates.keys())[0]
                                        times_cached = self.asc_dates.get(asc_date_str,[])
                                        if times_cached:
                                            asc_time_str = sorted(times_cached)[0]
                                            found = True
                                            self.logger(f"⚡ ASC desde cache: {asc_date_str} {asc_time_str}", self.account_id, self.config.email)
                                    if not found:
                                        try:
                                ...
                                p,url,key=self._proxy_meta
h={"apikey":key,"Authorization":f"Bearer {key}","Content-Type":"application/json"}
requests.patch(f"{url}/rest/v1/proxies?id=eq.{p['id']}",headers=h,json={"fail_count":p["fail_count"]+1})
self.logger(f"❌ Error BOOK: ProxyError | fail++").__name__}", self.account_id, self.config.email)
                                self.change_proxy()
                                continue
