...
DEFAULT_HEADERS = {
    "Host": HOST,
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/20.0.0.0 YaBrowser/24.1.0.0 Safari/537.36",
    "sec-ch-ua": '"Not_A Brand";v="8", "Chromium";v="20", "YaBrowser";v="24.1", "Yowser";v="2.5"',
    "sec-ch-ua-mobile": "?0",
...
def get_speed_mode():
    ...
    weekday = now.weekday()  # 0=Lun 1=Mar 2=Mie 3=Jue 4=Vie 5=Sab 6=Dom

    # MIERCOLES — BrightData residencial + ULTRA
    if weekday == 2:
        if h < 7:                              return "CONSERVADOR",  60.0, 1
        if h == 7 and m <= 50:                 return "RAPIDO",       10.0, 1
        if (h == 7 and m >= 51) or (h <= 10): return "ULTRA",         0.3, 2
        return "NORMAL", 20.0, 1

    # TODOS LOS DEMAS DIAS — DC proxies, sesion 8 min
    return "NORMAL", 0.2, 1
...
# ═══ TELEGRAM ALERTAS ═══
def send_telegram_alert(msg, token=None, chat_id=None):
    import os, requests as _req
    _token = token or os.getenv('TELEGRAM_BOT_TOKEN','')
    _chat  = chat_id or os.getenv('TELEGRAM_CHAT_ID','')
    if not _token or not _chat: return
    try:
        _req.post(f'https://api.telegram.org/bot{_token}/sendMessage',
            json={'chat_id':_chat,'text':msg,'parse_mode':'Markdown'}, timeout=8)
    except: pass


# ═══ BRIGHTDATA PROXY — Sticky session por cuenta ═══
import hashlib as _hashlib
BD_CUSTOMER = 'hl_d36aad6c'
BD_ZONE     = 'residential_proxy2'
BD_PASS     = 'yrkzbddify53'
BD_HOST     = 'brd.superproxy.io'
BD_PORT     = 33335
SESSION_TTL = 600  # 10 minutos

def build_bd_proxy(account_index: int, country: str = 'us', force_rotate: bool = False) -> str:
    epoch = int(time.time()) if force_rotate else int(time.time()) // SESSION_TTL
    raw   = f"nexus_{account_index}_{epoch}"
    sess  = _hashlib.md5(raw.encode()).hexdigest()[:12]
    user  = f"brd-customer-{BD_CUSTOMER}-zone-{BD_ZONE}-country-{country}-session-{sess}"
    return f"http://{user}:{BD_PASS}@{BD_HOST}:{BD_PORT}"
...
class ThreadLogger:
    ...
    def __call__(self, message, account_id=None, email=None):
        ...
        _skip = ["SID conocido","Init csrf","🔐 LOGIN","Get current","Solicitando página","Login exitoso","Rotando...","🔄 🏠","🔄 🏢","ProxyError","ConnectionError","JSONDecodeError","HTTPError"]
        if any(s in message for s in _skip):
            return
...
class Bot:
    def __init__(self, config, logger, account_id):
        ...
        self.current_proxy_index = _rnd.randint(0, max(0, len(self.proxies_list)-1))
        ...
        self._last_login_ts = time.time()  # evitar login doble al arrancar
    ...
    def get_brightdata_proxy(self, force_rotate=False):
        """BrightData sticky session — usar en miércoles"""
        from datetime import datetime as _dt
        acct_idx = (self.account_id or 0) % 50
        return build_bd_proxy(acct_idx, country='us', force_rotate=force_rotate)
    ...
    def init(self):
        self._initialize_proxy_session()
        self.login()
        # ⚡ Si ya tiene schedule_id conocido → saltar init_current_data
        # evita redirect /unsupported en cuentas con cita existente
        if self.config.schedule_id:
            self.logger("⚡ SID conocido — saltando init_current_data", self.account_id, self.config.email)
        else:
            self.init_current_data()
        self.init_csrf_and_cookie()

    def login(self):
        # ⚡ LOGIN CON PROXY RESIDENCIAL — VPS IP bloqueada por AIS
        _proxy_backup = self.session.proxies.copy() if hasattr(self.session, 'proxies') else {}
        # NO limpiar proxies — usar el proxy activo
        self.logger("🔐 LOGIN", self.account_id, self.config.email)
        log_traza(supabase, self.account_id, self.config.email, "info", "Solicitando página de login")
        r = self.session.get(f"{self.url}/users/sign_in",
            headers={COOKIE_HEADER: "", REFERER: f"{self.url}/users/sign_in", **DOCUMENT_HEADERS}, timeout=10)
        r.raise_for_status()
        ...
        r2 = self.session.post(f"{self.url}/users/sign_in",
            ...
            data=urlencode({"user[email]": self.config.email, "user[password]": self.config.password,
                "policy_confirmed": "1", "commit": "Sign In"}), timeout=10)
        # DETECTOR HTTP — acciones por código
        HTTP_ACTIONS = {
            200: ("ok",    None),
            302: ("ok",    None),
            401: ("block", "No autorizado (401)"),
            402: ("block", "Cuenta bloqueada AIS (402)"),
            403: ("proxy", "IP bloqueada por AIS (403)"),
            404: ("abort", "Cuenta no encontrada (404)"),
            429: ("wait",  "Rate limit AIS (429)"),
            500: ("retry", "Error servidor AIS (500)"),
            503: ("retry", "AIS no disponible (503)"),
        }
        code = r2.status_code
        action, alert_msg = HTTP_ACTIONS.get(code, ("unknown", f"HTTP {code} inesperado"))

        if action != "ok":
            import re as _re
            pattern = "locked until"
            txt = r2.text
            idx = txt.lower().find(pattern)
            lock_until = txt[idx+len(pattern):idx+len(pattern)+50].strip().split("<")[0].strip() if idx>0 else None
            full_alert = f"[{code}] {alert_msg}" + (f" hasta {lock_until}" if lock_until else "")

            self.logger(f"🚨 {full_alert}", self.account_id, self.config.email)
            log_traza(supabase, self.account_id, self.config.email, "error", full_alert,
                     {"code": code, "action": action, "lock_until": lock_until})
            ...
            if action == "block":
                ...
                raise Exception(f"ACCOUNT_BLOCKED_{code}")

            elif action == "proxy":
                self.change_proxy()
                raise Exception(f"PROXY_BLOCKED_{code}")

            elif action == "wait":
                import time as _t; _t.sleep(300)
                raise Exception(f"RATE_LIMITED_{code}")

            elif action in ["abort", "unknown"]:
                try:
                    supabase.table(TABLE_NAME).update({
                        "is_active": False,
                        "status": f"❌ {full_alert}"
                ...
                raise Exception(f"LOGIN_FAILED_{code}")
        ...
        # VALIDACION REAL — AIS usa AJAX (data-remote=true)
        # Login exitoso = JS con window.location.href="/en-do/niv/account"
        # Login bloqueado = HTML con "locked until"
        # Login rechazado = HTML con formulario de login visible
        _txt  = r2.text
        _html = _txt.lower()

        # 1. LOGIN EXITOSO — JS redirect a /account (caso normal AIS AJAX)
        _login_ok = (
            'window.location' in _txt and '/account' in _txt
        ) or (
            'niv/account' in _txt
        )
        ...
        if _login_ok:
            ...
            log_traza(supabase, self.account_id, self.config.email, "success", "Login exitoso")
            return

        # 2. CUENTA BLOQUEADA — AIS devuelve 200 + HTML con mensaje de bloqueo
        if "locked until" in _html or "your account is locked" in _html:
            _idx = _txt.lower().find("locked until")
            _until = _txt[_idx+len("locked until"):_idx+len("locked until")+60].strip().split("<")[0].strip() if _idx>0 else "tiempo desconocido"
            _msg = f"Bloqueada AIS hasta {_until}"
            self.logger(f"🔒 {_msg}", self.account_id, self.config.email)
            log_traza(supabase, self.account_id, self.config.email, "error", _msg, {"lock_until": _until})
            try:
                supabase.table(TABLE_NAME).update({
                    "is_active": False,
                    "status": f"🔒 {_msg}",
            ...
            send_telegram_alert(
                f"🔒 *CUENTA BLOQUEADA AIS*\n"
                f"📧 `{self.config.email}`\n"
                f"📝 {_msg}\n"
                f"🕐 {datetime.now().strftime('%d/%m %H:%M')}"
            )
            raise Exception(f"ACCOUNT_LOCKED: {_msg}")

        # 3. CREDENCIALES INVALIDAS — formulario visible sin bloqueo
        if "new_user" in _html and "sign_in_form" in _html and "locked" not in _html:
            _msg = "Credenciales invalidas"
            self.logger(f"❌ {_msg}", self.account_id, self.config.email)
            try:
                supabase.table(TABLE_NAME).update({
                    "is_active": False,
            ...
            raise Exception(f"INVALID_CREDENTIALS")

        # 4. CAPTCHA
        if any(x in _html for x in ["captcha", "recaptcha", "verify you are human"]):
            self.logger(f"🤖 Captcha detectado", self.account_id, self.config.email)
            raise Exception("CAPTCHA_DETECTED")

        # 5. DESCONOCIDO — loguear y continuar con cautela
        self.logger(f"⚠️ Respuesta login no reconocida — asumiendo OK", self.account_id, self.config.email)
        log_traza(supabase, self.account_id, self.config.email, "warning", "Respuesta login no reconocida",
                 {"preview": _txt[:200]})

    def init_current_data(self):
        self.logger("Get current appointment", self.account_id, self.config.email)
        # Ir directo al grupo con schedule_id para evitar redirect /unsupported
        group_url = f"{self.url}/groups/{self.config.schedule_id}"
        r = self.session.get(group_url, headers={**self.headers(), **DOCUMENT_HEADERS}, timeout=10)
        r.raise_for_status()
        self.logger(f"📄 Status: {r.status_code} | URL: {r.url[:60]}", self.account_id, self.config.email)
        # Si redirige a unsupported intentar con /niv directo
        if 'unsupported' in str(r.url):
    ...
    def load_change_appointment_page(self):
        r = self.session.get(f"{self.url}/schedule/{self.config.schedule_id}/appointment",
            headers={**self.headers(), **DOCUMENT_HEADERS, **SEC_FETCH_USER_HEADERS,
                REFERER: f"{self.url}/schedule/{self.config.schedule_id}/continue_actions"}, timeout=10)
        r.raise_for_status()
        return r
    ...
    def book(self, avail_date, avail_time, asc_date=None, asc_time=None):
        ...
        return self.session.post(f"{self.url}/schedule/{self.config.schedule_id}/appointment",
            ...
            data=urlencode(body), timeout=20)
        ...
        while not self.cita_programada:
            try:
                ...
                while not self.cita_programada:
                    ...
                    if time.time() - last_csrf_refresh >= 180:
                        try:
                            self.init_csrf_and_cookie()
                            last_csrf_refresh = time.time()
                        except: pass
                    ...
                    if _now - _last_login >= 480:  # 8 minutos
                        try:
                            ...
                            self.logger(f"⚠️ Re-login fallido: {type(_le).__name__}", self.account_id, self.config.email)
                    ...
                    try:
                        ...
                        if any(x in err_str for x in ["401","402","403","sign_in","csrf","login","session"]):
                            # Sesion expirada — renovar CSRF sin hacer login completo
                            self.logger("🔄 Sesion expirada — renovando CSRF", self.account_id, self.config.email)
                            try:
                                self.init_csrf_and_cookie()
                                last_csrf_refresh = time.time()
                            except:
                                break  # Solo hacer re-login si CSRF falla tambien
                        else:
                            self.change_proxy()
                            continue
                    ...
                    # LOGICA: en rango min/max + ANTES de cita actual (mejorar) o cualquiera si no tiene cita
                                ...
                                if "appointment/instructions" in rb.url:
                                    ...
                                    if self.appointment_datetime and self.appointment_datetime.date() == datetime.strptime(date_str, "%Y-%m-%d").date():
                                        ...
                                        send_telegram_alert(
                                            f"🎉 *¡¡CITA AGENDADA!!*\n"
                                            f"📧 `{self.config.email}`\n"
                                            f"📅 {self.appointment_datetime.strftime('%Y-%m-%d %H:%M') if self.appointment_datetime else 'Ver sistema'}\n"
                ...
                if err_name == "NoScheduleIdException":
                    self.logger(f"📵 SIN ACCESO AL CALENDARIO: {self.config.email} — pausa 30min", self.account_id, self.config.email)
                    try:
                        supabase.table(TABLE_NAME).update({
                            "is_active": False,
                            "status": "Sin acceso calendario"
                        }).eq("id", self.account_id).execute()
                    except: pass
                    return  # el watchdog la reactivará
                    time.sleep(3600)
                    return
                self.change_proxy()
                time.sleep(0.1)
...
def run_account_thread(account_data, logger):
    ...
    while True:
        try:
            ...
            # CUENTA BLOQUEADA POR AIS — pausar y salir inmediatamente
            if any(x in err for x in ['ACCOUNT_LOCKED', 'ACCOUNT_DISABLED', 'INVALID_CREDENTIALS']):
                logger(f"🔒 Cuenta pausada por bloqueo AIS: {email}", account_id, email)
                try:
                    supabase.table(TABLE_NAME).update({
                        "is_active": False,
                        "status": f"🔒 {err[:60]}"
                ...
                return  # NO reintentar — watchdog la reactivará

            # Sin schedule ID → pausa 10 min
            if 'NoScheduleId' in err or 'unsupported' in err.lower():
                logger(f"⏸ Sin calendario — pausa 10min: {email}", account_id, email)
                try:
                    supabase.table(TABLE_NAME).update({
                        "is_active": False,
                        "status": "📵 Sin acceso calendario"
            ...
            # Proxy/Red → cambiar proxy y continuar sin contar error
            if any(x in err for x in ['PROXY_BLOCKED', 'ProxyError', 'ConnectionError']):
                import time as _t; _t.sleep(2)
                continue

            # Errores genéricos seguidos → pausa 5 min
            logger(f"❌ Error ({errores_seguidos}/{MAX_ERRORES}): {err[:60]}", account_id, email)
            if errores_seguidos >= MAX_ERRORES:
                logger(f"⏸ Pausa 5min: {email}", account_id, email)
                import time as _t; _t.sleep(300)
                errores_seguidos = 0
                try:
                    r = supabase.table(TABLE_NAME).select("*").eq("id", account_id).execute()
                    if r.data: account_data = r.data[0]
                except: pass
                continue
...
def get_active_accounts(logger):
    try:
        ...
        cuentas = r.data if r.data else []
        # Auto-proxy: si cuenta sin proxies → asignar del pool global
        pool_ref = next((c.get("proxies","") for c in cuentas if (c.get("proxies") or "").strip()), None)
        if pool_ref:
            for c in cuentas:
                if not (c.get("proxies") or "").strip():
                    supabase.table(TABLE_NAME).update({"proxies": pool_ref}).eq("id", c["id"]).execute()
                    c["proxies"] = pool_ref
                    logger(f"⚡ Auto-proxy asignado: {c.get('email','')[:30]}")
        return cuentas
    except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print('Nexus ya está corriendo. Saliendo.')
        sys.exit(1)
    try:
        main()
